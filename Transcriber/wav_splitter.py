...
def split(data, seconds_pr_split=None, overlap=None):
    if(seconds_pr_split == None):
        interval = data.meta.framerate # =1s
    else:
        interval = data.meta.framerate*seconds_pr_split
    if(overlap == None):
        overlap = 0
    ...
    for i in range(iterations):
        start = i * interval
        end = start + interval + (data.meta.framerate * overlap)
        canned.append(slicewave(data, start, end))
    newdata = combine(canned)
    return newdata, iterations
